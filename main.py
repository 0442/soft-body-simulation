...
class Node:
    ...
    def update_state(self):
        ...
        vel = self.__velocity
        pos = self.__position
        force_sum = reduce(lambda a, b: (a[0]+b[0], a[1]+b[1]), self.__forces.values(), (0, 0))
...
class Edge:
    ...
    def calculate_damping_vectors(self):
        ...
        """
        f1, f2 = self.calculate_force_vectors()
...
class Simulation:
    def __init__(self, bounce_damping:float=0.8, friction_coeff:float=0.5) -> None:
        ...
        self.__bodies:set[SoftBody] = set()
    ...
    def collision_detection(self):
        for body in self.__bodies:
            for node in body.nodes:
                ...
                # floor
                if y > disp_h:
                    friction = (-Utils.num_sign(v[0]) * node.mass * self.__friction_coeff, 0)
                    if abs(v[0]) < 1:
                        friction = (0,0)
                        v = (0,0)
                ...
                # right wall
                if x > disp_w:
                    friction = (0, -Utils.num_sign(v[1]) * node.mass * self.__friction_coeff)
                    if abs(v[1]) < 1:
                        friction = (0,0)
                        v = (0,0)
    ...
    def auto_run(self, time_scale:float=1) -> None:
        ...
        while True:
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if k[pygame.K_SPACE] == True:
                        ...
                        if is_paused == True: print("paused. [SPACE]")
                        else: print(">>")

                if k[pygame.K_MINUS] == True:
                    if time_scale > 0.01:
                        time_scale = round(time_scale - 0.01, 2)
            ...
            if is_paused == False:
                ...
                sleep(PhysConsts.time_step / time_scale)
...
if __name__ == "__main__":
    simulation = Simulation(bounce_damping=1, friction_coeff=100)
    ...
    node_list[0].set_velocity((0,20)) # add spin
    node_list[1].set_velocity((0,-20))




    for node in node_list: node.update_force("gravity", (0, node.mass*9.81*10))

    soft_body = SoftBody(node_list, 1000, 0.01, 1, edge_deform_deform=0.8, edge_tear_deform=1)#, edge_tear_deform=0.25)
    soft_body.add_velocity((30,0))
    soft_body.move_body((0.1,5.5))
    #soft_body.add_external_force("gravity", (0,0.1))
    soft_body.add_edge(0,1, rest_length="distance")
    soft_body.add_edge(0,2, rest_length="distance")
    soft_body.add_edge(2,3, rest_length="distance")
    soft_body.add_edge(3,1, rest_length="distance")

    soft_body.add_edge(0,3, rest_length="distance")
    soft_body.add_edge(1,2, rest_length="distance")

    simulation.add_body(soft_body)
